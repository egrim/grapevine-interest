...
template<typename T, size_t BitsPerBin = 4, size_t HashValues = 2,
...
public:
    ...
    typedef twohash_dynamic_counting_bloom_filter<T, BitsPerBin, HashValues,
...
public:
    ...
    twohash_dynamic_counting_bloom_filter() :
            bits(bucket_size(default_num_bins)), _num_bins(default_num_bins) {
    }

    explicit twohash_dynamic_counting_bloom_filter(const size_t requested_bins) :
            bits(bucket_size(requested_bins)), _num_bins(requested_bins) {
    }

    template<typename InputIterator>
    twohash_dynamic_counting_bloom_filter(const InputIterator start,
            const InputIterator end) :
            bits(bucket_size(std::distance(start, end) * 4)), _num_bins(
                    std::distance(start, end) * 4) {
    ...
    //! auxiliary ops
    void clear() {
        for (bucket_iterator i = bits.begin(), end = bits.end(); i != end; ++i)
            *i = 0;
    }
...
template<class T, size_t BitsPerBin, size_t HashValues,
        ...
        class ExtensionFunction, typename Block, class Allocator>
void swap(
        twohash_dynamic_counting_bloom_filter<T, BitsPerBin, HashValues,
                ExpectedInsertionCount, HashFunction1, HashFunction2,
...
template<class T, size_t BitsPerBin, size_t HashValues,
        ...
        class ExtensionFunction, typename Block, class Allocator>
bool operator==(
        const twohash_dynamic_counting_bloom_filter<T, BitsPerBin, HashValues,
                ExpectedInsertionCount, HashFunction1, HashFunction2,
                ExtensionFunction, Block, Allocator>& lhs
        ,
        const twohash_dynamic_counting_bloom_filter<T, BitsPerBin, HashValues,
